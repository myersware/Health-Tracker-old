...
export class TodoStore {
    ...
    constructor(private todoBackendService: TodoStorageService) {
    ...
    loadInitialData() {
        this.todoBackendService.getAllTodos()
            .subscribe(
                res => {
                    let todos = (<Object[]>JSON.parse(res)).map((todo: any) =>
    ...
    addTodo(newTodo: Todo): Observable<Todo> {
        ...
        obs.subscribe(
            res => {
                //console.log("addTodo: old size=" + this._todos.getValue().size);
                this._todos.next(this._todos.getValue().push(newTodo));
                //console.log("addTodo: new size=" + this._todos.getValue().size);
            });
